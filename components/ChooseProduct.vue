...
<template>
  <div px-3 min-h-xl gap-x-4 flex stretch>
    <div w="50%" grow h-fit border-gray-300 shadow-xl border p-3 rounded-xl>
      ...
      <div id="comboscroll" as="div" rounded-md max-h-xl h-fit rounded-lg overflow-hidden static mt-2>
        <table class="divide-y w-full rounded-lg  divide-sky-300">
          ...
          <tbody class="divide-y cursor-pointer divide-sky-200 bg-white">
            <tr v-for="(product, index) in filteredProducts" :key="product.id" hover="bg-sky-100" :class="index % 2 === 0 ? 'bg-sky-50' : 'bg-white'" border-collapse @click="store.addToOrderItems(product)">
